...
END
  ...
  structures in that set. (Use XYZ_TRAINING to write the xyz files for
  a training set.)
...
END
  ...
  write only the din file for that set. Use method method.s/method.i
  to write the reference values, or 0 if method is not given. (Use
  DIN_TRAINING to write the din files for a training set.)
