...
END
  ...
  where structurex.s are structure idenfiers from the database and
  valuex.r are the calculated values (typically, energies) for those
  structures. The structure names are the same as root of the file
  names generated using WRITE, so this file can be easily generated
  with utilities such as grep or awk.
  ...
  method.s, which is taken as reference for the comparison. If ACP is
  also present, then compare to the linear model prediction using the
  given ACP. The acp.s can be an ACP name or a file. Using the ACP
  keyword requires a training set and that the necessary data is
  available in the database.

  If INSERT is present, insert the calculated properties into the
  database as the evaluation of method method.s.
