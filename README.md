...
| [Generating ACPs using training set data](#generating-acps-using-the-training-set-data)                 | TRAINING GENERATE                                                                                                                                                                                                    |
| [Calculating training set maximum coefficients](#calculation-of-training-set-maximum-coefficients)      | TRAINING MAXCOEF                                                                                                                                                                                                     |
...
If TERM is used then the training set must be defined. In this case,
the data file (`file.s`) must contain entries of the form:
~~~
structure1.s value1.r value2.r ...
...
structure:
~~~
symbol.s  l.s  exp.r  exprn.i  maxcoef.r  property.s
~~~
where `symbol.s` is the atomic symbol, `l.s` is the angular momentum
label, `exp.r` is the exponent, `exprn.i` is the rn-exponent,
and `maxcoef.r` is the maximum
...
DELETE TERM [method.s prop.s atom.s l.i exp.r] ...
DELETE MAXCOEF
~~~
...
be given in order: method key, property key, atomic symbol, angular
